...
from ..services.disease_service import get_disease_details_mapped
...
async def get_disease(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Fetch disease details from MongoDB/local data with transformation
        print(f"Fetching disease details for: {prediction['disease']}")
        disease_details = await get_disease_details_mapped(prediction["disease"])
        print(f"Disease details found: {disease_details is not None}")

        if disease_details:
            # Merge model confidence with disease details
